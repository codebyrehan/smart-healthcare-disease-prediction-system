...
def eda():
    ...
    try:
        return jsonify({"rows": len(DATASET), "statistics": summary_statistics(DATASET).round(4).to_dict(orient="records"), "correlation": correlation_matrix(DATASET).round(4).fillna(0).to_dict(), "outcomes": outcome_summary(DATASET).round(2).to_dict(orient="records"), "feature_by_outcome": feature_by_outcome(DATASET).round(4).to_dict(orient="records")})
...
def evaluation():
    """Return evidence generated on the fixed stratified test split during training."""
...
def report():
    ...
    body = f"<!doctype html><html lang='en'><head><meta charset='utf-8'><title>Smart Healthcare Analysis Report</title><style>body{{font-family:Arial,sans-serif;max-width:1000px;margin:40px auto;padding:0 20px}}table{{border-collapse:collapse;width:100%}}th,td{{border:1px solid #ddd;padding:8px;text-align:left}}.note{{padding:12px;background:#f4f6f8}}</style></head><body><h1>Smart Healthcare — Data Science Report</h1><p>Validated PIMA Indians Diabetes Dataset · {len(DATASET)} records · {len(FEATURES)} predictive features</p><h2>Outcome distribution</h2><ul>{classes}</ul><h2>Descriptive statistics</h2><table><thead><tr><th>Feature</th><th>Mean</th><th>Std</th><th>Min</th><th>Max</th></tr></thead><tbody>{rows}</tbody></table><p class='note'><strong>Responsible use:</strong> Educational machine-learning analysis only. Model predictions do not constitute a medical diagnosis or treatment recommendation.</p></body></html>"
...
def sensitivity():
    ...
    try:
        ...
        return jsonify({"feature": feature, "results": sensitivity_analysis(MODEL, clean_baseline, feature, clean_values), "disclaimer": "Sensitivity analysis describes model behavior; it does not establish causation or provide medical advice."})
