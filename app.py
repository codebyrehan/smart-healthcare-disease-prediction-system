...
def build_model_registry():
    """Build all three portfolio models once so the UI can switch models reliably."""
    if DATASET is None:
        return {}
    from sklearn.ensemble import RandomForestClassifier
...
@app.get("/api/health")
def health():
    return jsonify({"status": "ok", "model_loaded": MODEL is not None, "dataset_loaded": DATASET is not None, "available_models": list(MODEL_ALIASES)})
...
def report():
    ...
    body = f"<!doctype html><html lang='en'><head><meta charset='utf-8'><title>Smart Healthcare Analysis Report</title></head><body><h1>Smart Healthcare — Data Science Report</h1><p>Validated PIMA Indians Diabetes Dataset · {len(DATASET)} records · {len(FEATURES)} predictive features</p><h2>Outcome distribution</h2><ul>{classes}</ul><h2>Descriptive statistics</h2><table><thead><tr><th>Feature</th><th>Mean</th><th>Std</th><th>Min</th><th>Max</th></tr></thead><tbody>{rows}</tbody></table><p><strong>Responsible use:</strong> Educational machine-learning analysis only.</p></body></html>"
...
def sensitivity():
    ...
    try:
        ...
        return jsonify({"feature": feature, "model": model_name, "results": sensitivity_analysis(MODEL_REGISTRY[key], clean_baseline, feature, clean_values), "disclaimer": "Sensitivity analysis describes model behavior; it does not establish causation or provide medical advice."})
